...
public sealed class DesktopDuplicator : IDisposable
{
    ...
    // presents/s (surfaced downstream) = Δaccumulated (it includes the acquired present) tells
    // "the content is only N fps" apart from "the pipeline is dropping presents".
