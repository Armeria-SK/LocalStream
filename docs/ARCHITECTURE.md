...
| NVIDIA-only encoder coupling (GameStream/Sunshine quirks) | **NVENC-preferred with neutral fallback** — direct NVENC ULL on NVIDIA (cleanest field logs), automatic Media Foundation fallback for every other GPU or any NVENC init/registration failure. |
...
DXGI Desktop Duplication (present-driven, GPU texture)     1–3 ms
  ...
  → NVENC ULL (NVIDIA, preferred) or Media Foundation H.264   3–8 ms
